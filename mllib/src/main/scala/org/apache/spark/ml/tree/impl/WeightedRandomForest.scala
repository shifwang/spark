 ...
 */
private[spark] object WeightedRandomForest extends Logging with Serializable {

  /**
   * Reservoir weighted sampling with input size
   *
   ...
   */
  def reservoirWeightedSampling[T: ClassTag](
    input : Iterator[T],
    k : Int,
    weight : Array[Double],
    seed : Long = Random.nextLong()
      ...
      //val reservoir = mutable.PriorityQueue[T]()
      ...
      //val newReservoir = new Array[T](weight.length)
      //System.arraycopy(reservoir, 0, newReservoir, 0, weight.length)
...
/**
  ...
  def runBagged(input: RDD[Instance],  
      ...
      prune: Boolean = true, // exposed for testing only, real trees are always pruned, Abhi changed it to false. 
    ...
    while (nodeStack.nonEmpty) {
      ...
      WeightedRandomForest.selectNodesToSplit(nodeStack, maxMemoryUsage, metadata, rng)
  ...
  def run(
    ...
    val metadata = WeightedDecisionTreeMetadata
      .buildMetadata(input.retag(classOf[Instance]), strategy, numTrees, featureSubsetStrategy, featureWeight)
  ...
  private def mixedBinSeqOp(
      ...
      numSamples: Int,
      sampleWeight: Double,
      featuresForNode: Option[Array[Int]],seed: Long, dataSubSamplingRate: Double): Unit = {
  ...
  private[tree] def findBestSplits(
    ...
    def nodeBinSeqOp(
        ...
        splits: Array[Array[Split]],consider : Boolean,seed : Long,dataSubSamplingRate: Double): Unit = {
      if (nodeInfo != null) {
        val aggNodeIndex = nodeInfo.nodeIndexInGroup
        val subsampleRand = new XORShiftRandom(seed + aggNodeIndex.toLong)
        var dropSample = false
        ...
        val numSamples = baggedPoint.subsampleCounts(treeIndex)
         if(numSamples == 0){
            ...
            if(subsampleRand.nextDouble() > dataSubSamplingRate){
                dropSample = true
            }
        }
        ...
        if (metadata.unorderedFeatures.isEmpty) {
          orderedBinSeqOp(agg(aggNodeIndex), baggedPoint.datum, numSamples, sampleWeight,
            featuresForNode,seed + aggNodeIndex.toLong + Random.nextLong() ,dataSubSamplingRate)
        } else {
          mixedBinSeqOp(agg(aggNodeIndex), baggedPoint.datum, splits,
            metadata.unorderedFeatures, numSamples, sampleWeight, featuresForNode,seed + aggNodeIndex.toLong+ Random.nextLong(),dataSubSamplingRate)
        }
    ...
    def binSeqOp(
      ...
      treeToNodeToIndexInfo.foreach { case (treeIndex, nodeIndexToInfo) =>
        ...
        nodeBinSeqOp(treeIndex, nodeIndexToInfo.getOrElse(nodeIndex, null), 
          agg, baggedPoint, splits,consider,seed + treeIndex.toLong ,dataSubSamplingRate) //seed is datapoint seed. 
      }
      agg
    }

    //null happens when there is no node for the data point being considered 
    ...
    def binSeqOpWithNodeIdCache(
      ...
      treeToNodeToIndexInfo.foreach { case (treeIndex, nodeIndexToInfo) =>
        ...
        nodeBinSeqOp(treeIndex, nodeIndexToInfo.getOrElse(nodeIndex, null),
          agg, baggedPoint, splits,consider,seed + treeIndex.toLong,dataSubSamplingRate)
    ...
    val partitionAggregates = if (useNodeIdCache) {
      ...
      input.zip(nodeIds).mapPartitionsWithIndex{ (pID,points) =>  //.sample(false,dataSubSamplingRate,rng.nextLong())
        ...
        val s = rng.nextLong()+ pID.toLong + 1L
        points.foreach(binSeqOpWithNodeIdCache(nodeStatsAggregators,_, bcSplits.value, dataSubSamplingRate, s))
      ...
      input.mapPartitionsWithIndex { (pID,points) => //.sample(false,dataSubSamplingRate,rng.nextLong())
        // Construct a nodeStatsAggregators array to hold node aggregate stats,
        // each node will have a nodeStatsAggregator
        val nodeStatsAggregators = Array.tabulate(numNodes) { nodeIndex =>
  ...
  private[tree] def binsToBestSplit(
    ...
    val numCandidates = if (featuresForNode.isDefined) {
        featuresForNode.get.length
    } else {
        binAggregates.metadata.numFeaturesPerNode
    }
    val validFeatureSplits =
      Iterator.range(0, numCandidates).map { featureIndexIdx =>
   ...
   *       The `numSplits` attribute in the `WeightedDecisionTreeMetadata` class will be set accordingly.
   *
  ...
  private[tree] def selectNodesToSplit(
    ...
    while (nodeStack.nonEmpty && !groupDone) {
      ...
      val nodeMemUsage = WeightedRandomForest.aggregateSizeForNode(metadata, featureSubset) * 8L
  ...
  private def samplesFractionForFindSplits(
      metadata: WeightedDecisionTreeMetadata): Double = {
